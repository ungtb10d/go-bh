// Package bh is a library for CLI Go applications to help interface with the bh CLI tool,
// and the GitHub API.
//
// Note that the examples in this package assume bh and git are installed. They do not run in
...
import (
	...
	iapi "github.com/ungtb10d/go-bh/internal/api"
	"github.com/ungtb10d/go-bh/internal/git"
	irepo "github.com/ungtb10d/go-bh/internal/repository"
	"github.com/ungtb10d/go-bh/pkg/api"
	"github.com/ungtb10d/go-bh/pkg/auth"
	"github.com/ungtb10d/go-bh/pkg/config"
	repo "github.com/ungtb10d/go-bh/pkg/repository"
	"github.com/ungtb10d/go-bh/pkg/ssh"
...
func Exec(args ...string) (stdOut, stdErr bytes.Buffer, err error) {
	path, err := path()
	if err != nil {
		err = fmt.Errorf("could not find bh executable in PATH. error: %w", err)
		return
	}
...
func run(path string, env []string, args ...string) (stdOut, stdErr bytes.Buffer, err error) {
	...
	err = cmd.Run()
	if err != nil {
		err = fmt.Errorf("failed to run bh: %s. error: %w", stdErr.String(), err)
		return
	}
	return
}
...
// and unix domain socket are resolved from the bh environment configuration.
