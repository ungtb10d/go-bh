...
func TestRun(t *testing.T) {
	stdOut, stdErr, err := run(os.Args[0],
		...
		"-test.run=TestHelperProcess", "--", "bh", "issue", "list")
	assert.NoError(t, err)
	assert.Equal(t, "[bh issue list]", stdOut.String())
...
func TestRunError(t *testing.T) {
	stdOut, stdErr, err := run(os.Args[0],
		[]string{"GH_WANT_HELPER_PROCESS=1"},
		"-test.run=TestHelperProcess", "--", "bh", "issue", "list", "error")
	assert.EqualError(t, err, "failed to run bh: process exited with error. error: exit status 1")
