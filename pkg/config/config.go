...
// Note: This package is intended for use only in bh, any other use cases are subject
...
import (
	...
	"github.com/ungtb10d/go-bh/internal/yamlmap"
)
...
// Config is a in memory representation of the bh configuration files.
...
// Read bh configuration files from the local file system and
// return a Config.
...
// Write bh configuration files to the local file system.
// It will only write bh configuration files that have been modified
...
func ConfigDir() string {
	...
	if a := os.Getenv(ghConfigDir); a != "" {
		...
		path = filepath.Join(b, "bh")
	} else if c := os.Getenv(appData); runtime.GOOS == "windows" && c != "" {
		path = filepath.Join(c, "GitHub CLI")
	} else {
		d, _ := os.UserHomeDir()
		path = filepath.Join(d, ".config", "bh")
	}
	return path
}
...
func DataDir() string {
	...
	if a := os.Getenv(xdgDataHome); a != "" {
		...
		c, _ := os.UserHomeDir()
		path = filepath.Join(c, ".local", "share", "bh")
	}
	return path
}
...
git_protocol: https
# What editor bh should run when creating issues, pull requests, etc. If blank, will refer to environment.
editor:
# When to interactively prompt. This is a global config that cannot be overridden by hostname. Supported values: enabled, disabled
prompt: enabled
# A pager program to send command output to, e.g. "less". Set the value to "cat" to disable the pager.
pager:
# Aliases allow you to create nicknames for bh commands
aliases:
  co: pr checkout
# The path to a unix socket through which send HTTP connections. If blank, HTTP traffic will be handled by net/http.DefaultTransport.
http_unix_socket:
# What web browser bh should use when opening URLs. If blank, will refer to environment.
browser:
`
